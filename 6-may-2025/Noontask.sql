...
department varchar(100)  , bossno int ,
foreign key (bossno) references emp(empno));

create table dept ( deptname varchar(100) primary key ,
deptfloor int not null check (deptfloor >= 0) , phoneno varchar(10) not null,empno int ,
foreign key (empno)
references emp(empno));

create table item ( itemname varchar(100) primary key,
itemtype varchar(100) not null , itemcolor varchar(100) not null);

create table sales ( salesno int primary key ,
qty int check (qty >= 1) default 1, itemname varchar(100) not null,
deptname varchar(100) not null , foreign key (deptname)
references dept(deptname) ,foreign key (itemname)
references item(itemname) );

alter table emp add constraint fk\_const foreign key (department)
...
-- Due to circular dependency we can have it a seperate table for employee and department mapping and rest others remains same 